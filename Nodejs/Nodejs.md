...
一个具备交互式确认的文件复制工具，核心流程如下：

## 流程

1. 从 `process.argv` 获取源路径和目标路径
2. 校验参数完整性，源文件是否存在
3. 判断目标路径状态：
   - 目标为已有目录 → 直接复制，保持原文件名
   - 目标为已有文件 → 询问是否覆盖
...
```js
const [copyPath, pastePath] = process.argv.slice(2)
...
// 目标为已有目录 → 复制到目录中
if (existsSync(pastePath) && statSync(pastePath).isDirectory()) {
  await fs.copyFile(copyPath, path.join(pastePath, path.basename(copyPath)))
}

// 目标为已有文件 → 交互确认覆盖
process.stdin.once('data', async (data) => {
  const input = data.toString().trim()
  if (input === 'y') await fs.copyFile(copyPath, pastePath)
  if (input === 'n') process.exit(0)
})

// 目标不存在 → 创建目录后复制
const { dir } = path.parse(pastePath)
await fs.mkdir(dir, { recursive: true })
await fs.copyFile(copyPath, pastePath)
```

> 涉及知识点：process.argv / stdin/stdout / existsSync / statSync / copyFile / mkdir / path.basename / path.join / path.parse

---

# 综合实践：Markdown 索引生成器

一个递归遍历目录、提取文件标题、生成 INDEX.md 的索引工具。
...
async function collectMdFiles(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      res.push(...await collectMdFiles(fullPath))
    } else if (entry.isFile() && entry.name.endsWith('.md')) {
      res.push(fullPath)
    }
  }
...
正则 `/^#\s+(.+)/m`：m 标志使 ^ 匹配行首，匹配以 `# ` 开头的行并捕获标题文本。未匹配到标题时 fallback 到文件名。
...
组合 Markdown 内容后通过 `fs.writeFile` 写入 `INDEX.md`。

> 涉及知识点：fs.readdir / readFile / writeFile / path.resolve / join / relative / dirname / basename / 递归 + 正则表达式
