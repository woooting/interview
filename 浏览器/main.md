...
1. **存储方式主要有三个**
   ...
   1. **localstorage**：便于客户端储存数据，在**本地保存**(需要手动删除)，可储存大小5MB以上，以键值对(Key-Value)的方式存储，多页面共享

   2. **sessionstorage**：不在不同的浏览器页面中共享，即使是同一个页面，数据在当前浏览器关闭后自动删除

   3. **cookie**：通过请求头发送给服务器，最大4kb，每次都会携带。
